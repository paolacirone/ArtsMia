...
public class ArtsmiaDAO {
	...
	/*public int getPeso(ArtObject a1, ArtObject a2) {
		...
		String sql = " SELECT COUNT(*) as peso " + "FROM exhibition_objects AS eo1, exhibition_objects eo2  "
				+ " WHERE eo1.exhibition_id =eo2.exhibition_id AND eo1.exhibition_id =?  AND eo2.exhibition_id =? ";
	...
	public List<Adiacenza> getAdiacenza(){
		...
		List<Adiacenza> a = new ArrayList<Adiacenza>();
		try {
			Connection conn = DBConnect.getConnection();
			PreparedStatement st = conn.prepareStatement(sql);
			ResultSet res = st.executeQuery();
			...
			while(res.next()) {
				a.add(new Adiacenza(res.getInt("obj1"), res.getInt("obj2"), res.getInt("peso")));
			}
